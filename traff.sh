...
if ip link show enX0 > /dev/null 2>&1; then
  ...
  for ip in $ip_addresses; do
    ...
    sudo docker run -d --network my_network_$i --name traff_$i traffmonetizer/cli_v2 start accept --token cCuCGOWZXNnk9dL5BR+cz1QHbjCdXJnFb8e3a9OAS2k= --device-name $ip
